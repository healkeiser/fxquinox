...
# Globals
ROOT_DIR_PATH = "C:/Users/valen/OneDrive/Projects/Code/VFX-UsdAssetResolver/files/implementations/CachedResolver"
SHOTS_DIR_PATH = os.path.join(ROOT_DIR_PATH, "workspace", "shots").replace(os.sep, "/")
ASSETS_DIR_PATH = os.path.join(ROOT_DIR_PATH, "workspace", "assets").replace(os.sep, "/")
ENTITY_TYPE_TO_DIR_PATH = {"shots": SHOTS_DIR_PATH, "assets": ASSETS_DIR_PATH}
...
class Resolver:
    ...
    def CreateRelativePathIdentifier(resolver, anchoredAssetPath, assetPath, anchorAssetPath):
        ...
        LOG.debug(
            "::: Resolver.CreateRelativePathIdentifier | {} | {} | {}".format(
                anchoredAssetPath, assetPath, anchorAssetPath
            )
        )
        ...
        remapped_relative_path_identifier = (
            f"{RELATIVE_PATH_IDENTIFIER_PREFIX}{entity_type}/{entity_identifier}?{entity_element}-{entity_version}"
        )
...
class ResolverContext:
    ...
    def ResolveAndCache(context, assetPath):
        ...
        LOG.debug("::: ResolverContext.ResolveAndCache | {} | {}".format(assetPath, context.GetCachingPairs()))
        resolved_asset_path = ""
        if assetPath.startswith(RELATIVE_PATH_IDENTIFIER_PREFIX):
            ...
            versionless_identifier = (
                f"{RELATIVE_PATH_IDENTIFIER_PREFIX}{entity_type}/{entity_identifier}?{entity_element}"
            )
            ...
            if mapping_hit:
                ...
                resolved_asset_path = os.path.normpath(
                    os.path.join(
                        ENTITY_TYPE_TO_DIR_PATH[entity_type],
                        entity_identifier,
                        "elements",
                        f"{entity_element}_{entity_version}.usd",
                    )
                )
        else:
            ...
            resolved_asset_path = os.path.join(
                ENTITY_TYPE_TO_DIR_PATH[entity_type], entity_identifier, f"{entity_identifier}_v002.usd"
            )
