...
def create_task(task_name: str, base_dir: str = ".", parent: QWidget = None) -> Optional[str]:
    ...
    # Create the task
    return _create_entity(_TASK, task_name, base_dir_path, parent)
...
def check_task(base_dir: str = ".") -> bool:
    ...
    return _check_entity(_TASK, base_dir)
...
def create_workfile(workfile_name: str, base_dir: str = ".", parent: QWidget = None) -> Optional[str]:
    ...
    # Create the task
    return _create_entity(_WORKFILE, workfile_name, base_dir_path, parent)
...
def _check_workfiles_directory(base_dir: str = ".") -> bool:
    ...
    return _check_entity(_WORKFILES_DIR, base_dir)
...
class FXProjectBrowserWindow(fxwidgets.FXMainWindow):
    ...
    def _handle_connections(self):
        ...
        self.tree_widget_assets.currentItemChanged.connect(self._get_current_asset)
        self.tree_widget_assets.currentItemChanged.connect(lambda: self._display_metadata(_ASSET))
        ...
        self.tree_widget_shots.currentItemChanged.connect(self._get_current_sequence_and_shot)
        self.tree_widget_shots.currentItemChanged.connect(self._populate_steps)
        self.tree_widget_shots.currentItemChanged.connect(self._handle_sequence_shot_selection)
        ...
        self.list_steps.currentItemChanged.connect(self._get_current_step)
        self.list_steps.currentItemChanged.connect(self._populate_tasks)
        self.list_steps.currentItemChanged.connect(lambda: self._display_metadata(_STEP))
        ...
        self.list_tasks.currentItemChanged.connect(self._get_current_task)
        self.list_tasks.currentItemChanged.connect(self._populate_workfiles)
        self.list_tasks.currentItemChanged.connect(lambda: self._display_metadata(_TASK))
        ...
        self.tree_widget_workfiles.currentItemChanged.connect(self._get_current_workfile)
        self.tree_widget_workfiles.currentItemChanged.connect(lambda: self._display_metadata(_WORKFILE))
    ...
    def _handle_sequence_shot_selection(self):
        ...
        if item_type == "shot":
            self._display_metadata(_SHOT)
        elif item_type == "sequence":
            self._display_metadata(_SEQUENCE)
        else:
            return
    ...
    def _populate_shots(self) -> None:
        ...
        for sequence in shots_dir.iterdir():
            ...
            # Set data
            sequence_item.setData(0, Qt.UserRole, _SEQUENCE)
            ...
            for shot in sequence.iterdir():
                ...
                # Set data
                shot_item.setData(0, Qt.UserRole, _SHOT)
    ...
    def _populate_steps(self):
        ...
        for step in workfiles_dir.iterdir():
            ...
            # Set data
            step_item.setData(Qt.UserRole, _STEP)
    ...
    def _populate_tasks(self):
        ...
        for task in tasks_dir.iterdir():
            ...
            # Set data
            task_item.setData(Qt.UserRole, _TASK)
    ...
    def _populate_workfiles(self):
        ...
        for workfile in workfiles_dir.iterdir():
            ...
            # Set data
            workfile_item.setData(0, Qt.UserRole, _WORKFILE)
    ...
    def _filter_workfiles_by_type(self, workfile_type):
        if workfile_type == "All":
            ...
            for i in range(self.tree_widget_workfiles.topLevelItemCount()):
                item = self.tree_widget_workfiles.topLevelItem(i)
                if item.text(0).endswith(".hip") or item.text(0).endswith(".hipnc") or item.text(0).endswith(".hiplc"):
                    item.setHidden(False)
                else:
                    item.setHidden(True)
            ...
            for i in range(self.tree_widget_workfiles.topLevelItemCount()):
                item = self.tree_widget_workfiles.topLevelItem(i)
                if item.text(0).endswith(".ma") or item.text(0).endswith(".mb"):
                    item.setHidden(False)
                else:
                    item.setHidden(True)
            ...
            for i in range(self.tree_widget_workfiles.topLevelItemCount()):
                item = self.tree_widget_workfiles.topLevelItem(i)
                if item.text(0).endswith(".nk") or item.text(0).endswith(".nknc"):
                    item.setHidden(False)
                else:
                    item.setHidden(True)
    ...
    def _on_filter_workfiles_by_type_changed(self):
        ...
        self._filter_workfiles_by_type(workfile_type)
        if workfile_type != "All":
    ...
    def _open_workfile(self):
        ...
        self.open_file_or_folder(path=item.data(1, Qt.UserRole))
    ...
    def _display_metadata(self, entity_type: str = None) -> None:
        ...
        if entity_type == _SEQUENCE and self.sequence:
            path = Path(self._project_root) / "production" / "shots" / self.sequence
        elif entity_type == _SHOT and self.sequence and self.shot:
            path = Path(self._project_root) / "production" / "shots" / self.sequence / self.shot
        elif entity_type == _ASSET and self.asset:
            path = Path(self._project_root) / "production" / "assets" / self.asset
        elif entity_type == _STEP and self.sequence and self.shot and self.step:
            path = (
                Path(self._project_root) / "production" / "shots" / self.sequence / self.shot / "workfiles" / self.step
            )
        elif entity_type == _TASK and self.sequence and self.shot and self.step and self.task:
            path = (
                Path(self._project_root)
                / "production"
                / "shots"
        ...
        elif entity_type == _WORKFILE and self.sequence and self.shot and self.step and self.task and self.workfile:
            path = (
                Path(self._project_root)
                / "production"
                / "shots"
    ...
    def _on_assets_context_menu(self, point: QPoint):
        ...
        action_show_in_file_browser = fxguiutils.create_action(
            ...
            lambda: self.open_file_or_folder(
                Path(self.tree_widget_shots.currentItem().data(1, Qt.UserRole)).parent.resolve().as_posix()
                if self.tree_widget_shots.currentItem()
                else get_project().get("FXQUINOX_PROJECT_ASSETS", None)
            ),
        )
...
class FXCreateTaskDialog(QDialog):
    ...
    def _create_task(self):
        ...
        task = create_task(task, step_dir, self)
        if not task:
            return
